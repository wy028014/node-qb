...
export class LoggerService {
    ...
    // 创建操作记录
    async create(createDto: LoggerCreateDto[]): Promise<{ success: Logger[], fail: LoggerCreateDto[] }> {
        if (createDto.length === 0) return { success: [], fail: [] };
        const result: { success: Logger[]; fail: LoggerCreateDto[]; } = { success: [], fail: [] };
        await this.loggerRepository.manager.transaction(async (entityManager: EntityManager) => {
    ...
    // 自定义查询操作记录
    async find(queryDto: LoggerQueryDto): Promise<{ list: Logger[]; total: number }> {
        const qb: SelectQueryBuilder<Logger> = this.loggerRepository.createQueryBuilder(`logger`);
        // 1) 软删除过滤
    ...
    async update(id: string, updateDto: LoggerUpdateDto): Promise<Logger> {
        const logger: Logger | undefined = await this.loggerRepository.preload({ id, ...updateDto });
        if (!logger) {
